...
# python3 /home/tractor/ros1_lawn_tractor_ws/project_notes/code_for_testing/path_gps_to_cartesian_v2.py
import json
import math
...
#Current Origin: origin_lat:=40.48524688166667, origin_lon:=-80.332720941667
...
for point in polygon_data['missionPolygon']:
    ...
    bearing = calculate_bearing(point['lat'], point['lng'], origin_lat, origin_lon)
    x, y = polar_to_cartesian(distance, bearing)
    y = -y  # Invert y-values to match ROS convention
    cartesian_polygon.append((x, y))

# Translate the coordinates so that the origin is at (0, 0)
origin_cartesian = cartesian_polygon[1]  # The new origin point is the second point in the list
