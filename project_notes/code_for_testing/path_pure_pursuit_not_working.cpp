                             ...
                             lookahead_frame_id_("lookahead")
{
  ...
  //pub_kdl_array = nh_.advertise<std_msgs::Float64MultiArray>("kdl_array", 1);
...
void PurePursuit::computeVelocities(nav_msgs::Odometry odom)
{
  ...
  try
  {
    geometry_msgs::Pose pose = path_.poses[idx_].pose;  // added to update the array
    geometry_msgs::Transform transform = tf.transform;  // added to update the array
    //std_msgs::Float64MultiArray kdl_array;              // Create a Float64MultiArray to hold the data
...
/*
        kdl_array.data.push_back(pose.position.x);          // Added for testing
        kdl_array.data.push_back(pose.position.y);
        kdl_array.data.push_back(pose.position.z);
        kdl_array.data.push_back(pose.orientation.x);
        kdl_array.data.push_back(pose.orientation.y);
        kdl_array.data.push_back(pose.orientation.z);
        kdl_array.data.push_back(pose.orientation.w);
        kdl_array.data.push_back(transform.translation.x);
        kdl_array.data.push_back(transform.translation.y);
        kdl_array.data.push_back(transform.translation.z);
        kdl_array.data.push_back(transform.rotation.x);
        kdl_array.data.push_back(transform.rotation.y);
        kdl_array.data.push_back(transform.rotation.z);
        kdl_array.data.push_back(transform.rotation.w);
        pub_kdl_array.publish(kdl_array);                   // Publish the array
*/
