#!/usr/bin/env python3
# python3 /home/tractor/ros1_lawn_tractor_ws/project_notes/code_for_testing/gui/gui_main_v2.py
...
class ROSLauncher(tk.Tk):
    ...
    def create_button(self, text, command):
        button = tk.Button(self, text=text, command=command, font=self.button_font)
        # Store the original button color
        original_color = button.cget("bg")
        button.bind("<Enter>", lambda event, b=button: b.config(bg="red"))
        button.bind("<Leave>", lambda event, b=button: b.config(bg=original_color))
        return button            

    # Sample launch functions for each button
    def launch_start(self):
        subprocess.Popen(["roslaunch", "YOUR_PACKAGE_NAME", "start.launch"])
