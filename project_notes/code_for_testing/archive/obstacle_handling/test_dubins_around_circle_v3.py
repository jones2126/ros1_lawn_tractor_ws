...
def calculate_shortest_circle_path(circle_center, radius, start_point, end_point, num_points=20):
    ...
    top_path_angle = end_angle - start_angle if end_angle >= start_angle else (2 * np.pi - start_angle) + end_angle
    bottom_path_angle = start_angle - end_angle if start_angle >= end_angle else (2 * np.pi - end_angle) + start_angle

    # Choose the shortest path
    if top_path_angle < bottom_path_angle:
        shortest_angle = top_path_angle
    else:
        shortest_angle = -bottom_path_angle  # Negative to indicate bottom path

    # Generate points along the shortest path
    circle_path = []
    for i in range(num_points + 1):
        angle = start_angle + (shortest_angle * i / num_points)
...
circle_center = (5, -0.7)  # Center of the obstacle
