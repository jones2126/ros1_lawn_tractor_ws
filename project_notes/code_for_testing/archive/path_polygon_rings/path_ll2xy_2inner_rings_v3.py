...
$ python3 /home/tractor/ros1_lawn_tractor_ws/project_notes/code_for_testing/archive/path_polygon_rings/path_ll2xy_2inner_rings_v3.py
...
def find_intersections_with_circle(xy_file_name, circle_center, radius, num_circle_points, output_file_path): 
    ...
    # Middle Segments
    qty_of_intersects = len(intersecting_index) - 1  # subtract 1 for zero indexing
    print("qty_of_intersects: ", qty_of_intersects)
    print("looping sequence for obstacle path")
    for i, path_index in zip(range(1, qty_of_intersects, 2), range(0, 4)):
        temp_path = xy_file_df.iloc[intersecting_index[i] + 1:intersecting_index[i + 1]].copy()
        point1 = intersection_points[i]
        point2 = intersection_points[i + 1]
        print(i, path_index, intersecting_index[i], intersecting_index[i+1])
    ...
    # End Segment
    temp_path = xy_file_df.iloc[intersecting_index[qty_of_intersects] + 1:].copy()
    revised_path = pd.concat([revised_path, temp_path], ignore_index=True)
