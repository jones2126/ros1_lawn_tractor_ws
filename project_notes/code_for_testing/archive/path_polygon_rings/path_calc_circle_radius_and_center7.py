...
$ python3 /home/tractor/ros1_lawn_tractor_ws/project_notes/code_for_testing/archive/path_polygon_rings/path_calc_circle_radius_and_center7.py
'''

print(f"Starting the script path_calc_circle_radius_and_center7.py...")
...
def save_to_xlsx(filename, sheet_name, reference_tag, center, radius, radius_best_fit):
    try:
    ...
    headers = ["Reference", "X", "Y", "Radius", "RadiusBestFit"]
    sheet.append(headers)

    formatted_center_x = round(center[0], 2)
    formatted_center_y = round(center[1], 2)
    formatted_radius = round(radius, 2)
    formatted_radius_best_fit = round(radius_best_fit, 2)

    print(f"Saving values: Reference={reference_tag}, X={formatted_center_x}, Y={formatted_center_y}, Radius={formatted_radius}, RadiusBestFit={formatted_radius_best_fit}")

    sheet.append([reference_tag, formatted_center_x, formatted_center_y, formatted_radius, formatted_radius_best_fit])
...
def read_obstacle_data_from_xlsx(filename, sheet_name):
    ...
    data.columns = ['Reference', 'X', 'Y', 'Radius', 'RadiusBestFit']
    reference = data['Reference'].values
    x = data['X'].values
    y = data['Y'].values
    radius = data['Radius'].values
    radius_best_fit = data['RadiusBestFit'].values
    ...
    print("Extracted Best Fit Radii:", radius_best_fit)

    obstacle_data = list(zip(reference, x, y, radius, radius_best_fit))
...
save_to_xlsx(xlsx_filename, result_sheet_name, tag_reference, (xc, yc), R_inflated, R)
print(f'Data saved to sheet {result_sheet_name} in {xlsx_filename}')
print(f"Circle Center: ({xc}, {yc}), Inflated Radius: {R_inflated}, Best Fit Radius: {R}")
...
num_points = 20
all_segments = []
for reference, x, y, radius, radius_best_fit in obstacle_data:
