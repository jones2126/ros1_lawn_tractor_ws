...
$ python3 /home/tractor/ros1_lawn_tractor_ws/project_notes/code_for_testing/archive/path_polygon_rings/path_ll2xy_2inner_rings_v2.py
...
def find_intersections_with_circle(xy_file_name, circle_center, radius, num_circle_points): 
    ...
    def plot_circle_and_intersections(center, radius, circle_segments, intersection_points, intersecting_segments, script_name, csv_file_path):
        ...
        if intersection_points:
            # Plotting the first point with label for legend
            plt.plot(intersection_points[0][0], intersection_points[0][1], 'ro', label='Intersection Points')
            # Plotting the rest without label
    ...
    def plot_xy_coordinates(csv_file):
        ...
        print(f"Number of rows in .csv file to be plotted: {len(data)}")
    ...
    # Segment 1
    temp_path = xy_file_df.iloc[0:intersecting_index[0]].copy()  # Copy rows into a new DataFrame
    i = 0
    ...
    circle_arc_df['Path_Index'] = 0                                 # Add a 'Path_Index' column with 0 since we are dealing with the inner most ring
    revised_path = pd.concat([revised_path, temp_path, circle_arc_df], ignore_index=True)

    # Segment 2
    temp_path = xy_file_df.iloc[intersecting_index[1]+1:intersecting_index[2]].copy()                # Copy rows into a new DataFrame
    i = 2  # incrementing by 2
    point1 = intersection_points[i]
    point2 = intersection_points[i + 1]
    circle_arc = calculate_shortest_path(circle_center, radius, point1, point2, num_circle_points)
    circle_arc_df = pd.DataFrame(circle_arc, columns=['X', 'Y'])    # Convert 'circle_arc' to DataFrame
    circle_arc_df['Path_Index'] = 1                                 # Add a 'Path_Index' column with 1 since I believe we are dealing with the next ring      
    revised_path = pd.concat([revised_path, temp_path, circle_arc_df], ignore_index=True)

    # Segment 3
    temp_path = xy_file_df.iloc[intersecting_index[3]+1:intersecting_index[4]].copy()              # Copy rows into a new DataFrame
    i = 4   # incrementing by 2
    point1 = intersection_points[i]
    point2 = intersection_points[i + 1]
    circle_arc = calculate_shortest_path(circle_center, radius, point1, point2, num_circle_points)
    circle_arc_df = pd.DataFrame(circle_arc, columns=['X', 'Y'])    # Convert 'circle_arc' to DataFrame
    circle_arc_df['Path_Index'] = 2                                 # Add a 'Path_Index' column with 2 since I believe we are dealing with the next ring       
    revised_path = pd.concat([revised_path, temp_path, circle_arc_df], ignore_index=True)

    # Segment 4
    temp_path = xy_file_df.iloc[intersecting_index[5]+1:intersecting_index[6]].copy()              # Copy rows into a new DataFrame
    i = 6   # incrementing by 2
    point1 = intersection_points[i]
    point2 = intersection_points[i + 1]
    circle_arc = calculate_shortest_path(circle_center, radius, point1, point2, num_circle_points)
    circle_arc_df = pd.DataFrame(circle_arc, columns=['X', 'Y'])    # Convert 'circle_arc' to DataFrame
    circle_arc_df['Path_Index'] = 3                                 # Add a 'Path_Index' column with 2 since I believe we are dealing with the next ring         
    revised_path = pd.concat([revised_path, temp_path, circle_arc_df], ignore_index=True)

    #Segment 5 
    temp_path = xy_file_df.iloc[intersecting_index[7]+1:].copy() # Copy rows into a new DataFrame
...
def main():
    ...
    intersections, segments, indices, circle_segments = find_intersections_with_circle(xy_file_name, circle_center, radius, num_circle_points)
