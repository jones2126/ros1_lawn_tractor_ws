...
def calculate_and_store_shortest_path(xlsx_file_path, all_intersection_data):
    if len(all_intersection_data) > 1:
        ...
        end_point = (all_intersection_data.iloc[1]['Intersection_X'], all_intersection_data.iloc[1]['Intersection_Y'])
        ...
        # Add the arc path data to the Excel file
        with pd.ExcelWriter(xlsx_file_path, engine='openpyxl', mode='a') as writer:
