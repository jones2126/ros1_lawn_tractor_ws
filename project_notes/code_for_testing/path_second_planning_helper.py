...
$ python3 ~/ros1_lawn_tractor_ws/project_notes/code_for_testing/path_second_planning_helper.py
...
# Loop to create, print and plot inner polygons
for i in range(1, ring_count + 1):
    inner_polygon = polygon.buffer(i * inset)
    ...
    x, y = inner_polygon.exterior.xy
    plt.plot(x, y, 'r-', label=f'Ring {i}')

# Show the plot
plt.xlabel('X')
plt.ylabel('Y')
plt.title('Polygon with Inner Rings')
