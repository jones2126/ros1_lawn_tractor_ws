...
# <!-- fix position 40.485509842 -80.332308247 357.9  the GPS antennae location -->
    ...
    def gps_callback(self, data):
        ...
        if self.prev_lat == 0 and self.prev_lon == 0:  # new block 8/28/24 10:06
            rospy.loginfo("Received first valid GPS data.")  # new line 8/28/24 10:06
            ...
            return  # Exit early as we can't calculate velocity yet
        ...
        if dt < 0.001:  # new block 8/28/24 10:06
            rospy.logwarn(f"Too small time difference: {dt}. Skipping this update.")  # new line 8/28/24 10:06
            return
        ...
        if dt > 0:  # Avoid division by zero 
            ...
            # Check for unreasonably high speeds
            if current_speed > self.max_allowed_speed:  # modified block 8/28/24 10:06
                rospy.logwarn(f"Detected abnormal speed: {current_speed} m/s. GPS status: {data.status.status}")  # modified line 8/28/24 10:06
                if len(self.prev_positions) >= 3:
                    ...
                    # Recalculate delta_lat and delta_lon
                    delta_lon = self.current_lon - self.prev_lon
                    delta_lat = self.current_lat - self.prev_lat
                else:
                    rospy.logwarn("Not enough historical data. Using previous position.")  # new line 8/28/24 10:06
            ...
            self.prev_positions.append((self.current_lat, self.current_lon))  # new line 8/28/24 10:06
            self.prev_headings.append(self.COG)  # new line 8/28/24 10:06
            self.prev_speeds.append(current_speed)  # new line 8/28/24 10:06
            
            # Keep only the last 3 entries
            self.prev_positions = self.prev_positions[-3:]  # new line 8/28/24 10:06
            self.prev_headings = self.prev_headings[-3:]  # new line 8/28/24 10:06
            self.prev_speeds = self.prev_speeds[-3:]  # new line 8/28/24 10:06

            self.gps_linear_velocity = current_speed
