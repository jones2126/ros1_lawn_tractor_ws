...
- Add timer nh.createTimer(ros::Duration(10.0), std::bind(&PurePursuit::checkForNewPath, &controller)); inside the constructor
- Change int main() so 'ros::spin();' starts the ROS event loop instead of function 'void PurePursuit::run()'
...
// PurePursuit() : ld_(1.0),v_max_(0.1),v_(v_max_),w_max_(1.0),pos_tol_(0.1),idx_(0),goal_reached_(true),nh_private_("~"),tf_listener_(tf_buffer_),
//     map_frame_id_("map"),robot_frame_id_("base_link"),lookahead_frame_id_("lookahead"),timer_(nh_.createTimer(ros::Duration(10.0),&PurePursuit::checkForNewPathTimerCallback, this)) {
...
private:
  ...
  //void checkForNewPathTimerCallback(const ros::TimerEvent&); // Private callback method

  // Private callback method for the timer
  void checkForNewPathTimerCallback(const ros::TimerEvent&) {
    std::cout << "going to checkForNewPath..." << std::endl;
    checkForNewPath();
  }
...
// $ rosparam set /pure_pursuit_steering_controller/new_input_path "/home/tractor/ros1_lawn_tractor_ws/project_notes/paths/Collins_Dr_62/ready_to_test/test_PID_output.txt"

void PurePursuit::checkForNewPath() {
  std::string new_input_file;
  if (ros::param::get("pure_pursuit_steering_controller/new_input_path", new_input_file) && !new_input_file.empty()) {
      // Load the new path
