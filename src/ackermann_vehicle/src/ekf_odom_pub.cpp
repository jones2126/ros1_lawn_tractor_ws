...
const double initialTheta = -1.509;   // point South outside from garage
...
double left_meters_travelled = 0.0;
double right_meters_travelled = 0.0;
...
void left_callback(const std_msgs::Float64& msg) {
    left_meters_travelled = msg.data;
}

void right_callback(const std_msgs::Float64& msg) {
    right_meters_travelled = msg.data;
}
...
void update_odom() {
  ...
  double cycleDistance = (right_meters_travelled + left_meters_travelled) / 2;
   
  // Calculate the number of radians the robot has turned since the last cycle
  double cycleAngle = asin((right_meters_travelled-left_meters_travelled)/WHEEL_BASE);
...
int main(int argc, char **argv) {
  ...
  ros::Subscriber left_sub = node.subscribe("/left_meters_travelled", 100, left_callback);
  ros::Subscriber right_sub = node.subscribe("/right_meters_travelled", 100, right_callback);
