#!/usr/bin/env python3
"""
cmd_vel_capture_odom.py

Sends cmd_vel statement based on file input and stores odom values in .txt file for analysis. 
...
An example command to access one element of the table that is loaded: print(mission_commands[2][1])  # row 2, element 1 - Remember 0
is the reference for the first element.
The .txt file needs at least two entries otherwise you will get the error: IndexError: invalid index to scalar variable.

sleep_time sets the rate at which twist messages will be published.  At 0.1 the measure Hz using rostopic hz equals 9.9 (i.e. ~10 HZ)
